...
for symbol, positions in symbols_index.items():
  for (x1, y1), (x2, y2) in itertools.combinations(positions, 2):
      ...
      pos = (x1, y1)
      while True:
          # Move in the direction of diff1
          pos = tuple(x + y for x, y in zip(pos, diff1))

          # Check if within the map boundaries
          if 0 < (pos[0] + 1) <= map_width and 0 < (pos[1] + 1) <= map_height:
              antinode_positions.append(pos)
          else:
              # As soon as we exceed the boundary, stop
              break

      pos = (x2, y2)
      while True:
          # Move in the direction of diff2
          pos = tuple(x + y for x, y in zip(pos, diff2))
